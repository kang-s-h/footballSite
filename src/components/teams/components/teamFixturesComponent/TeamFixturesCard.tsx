...
function TeamFixturesCard({ title }) {
  const { teamLastFixtures, teamNextFixtures } = dataStore();
  const { replaceRoundStr } = regExpStore();
  const [isLoading, setIsLoading] = useState(true);

  const fixturesKind = title === "last Fixtures" ? teamLastFixtures : teamNextFixtures;
                ...
                {fixturesKind?.map((fixture, index) => (
                  <div key={index} className="teamFixture_container_box">
                    ...
                    <div className="teamFixture_container_box-score">
                      {isLastFixtures ? `${fixture?.score?.fulltime?.home} - ${fixture?.score?.fulltime?.away}` : "VS"}
                    </div>
